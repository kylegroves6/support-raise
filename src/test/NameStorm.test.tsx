...
describe('duplicate detection', () => {
  it('shows a warning when first + last name matches an existing contact', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('duplicate-warning')).toHaveTextContent(/john smith is already/i)
    )
  })
  ...
  it('warns when entering someone who is the spouse in an existing couple entry', async () => {
    // "Kevin & Sue Smith" already exists — entering "Sue Smith" should warn
    const existing = [makeContact({ firstName: 'Kevin', lastName: 'Smith', organization: 'Kevin & Sue Smith' })]
    render(<NameStorm activeTrip={makeTrip()} contacts={existing} addContact={noop as never} onCreateTrip={noop} />)
    ...
    await waitFor(() =>
      expect(screen.getByTestId('duplicate-warning')).toHaveTextContent(/appears to be a spouse/i)
    )
  })
...
describe('space-to-advance from first name to last name', () => {
  it('focuses the last name field when Space is pressed after typing a first name', () => {
    render(<NameStorm activeTrip={makeTrip()} contacts={noContacts} addContact={noop as never} onCreateTrip={noop} />)
